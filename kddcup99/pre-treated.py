...
def preHandel_data():
    ...
    #将预处理结果转置
    #https://blog.csdn.net/qeqjkqjklq/article/details/109281091
    transposition_file='kddcup.data_10_T.csv'
    df = pd.read_csv(handled_file,header= None,low_memory=False)
    # df.values
    data = df.values
    data = list(map(list,zip(*data)))
    data = pd.DataFrame(data)
    data.to_csv(transposition_file,header=0,index=0)
